...
local function parse_resources(bs, peinfo, res)
    res = res or {}

    -- lookup the entry for the resource directory
    local dirTable = peinfo.OptionalHeader.Directory.ResourceTable
    if not dirTable then 
        return false, "ResourceTable directory not found" 
    end
    
    -- find the associated section
    local resourcedirectoryOffset = coff_utils.fileOffsetFromRVA(peinfo.Sections, dirTable.VirtualAddress)
    local bs = bs:range(dirTable.Size, resourcedirectoryOffset)
    ...
    local function readResourceDirectory(bs, resdir, level, tab)
        ...
        resdir.isDirectory = true;
        resdir.Characteristics = bs:readUInt32();          
        resdir.TimeDateStamp = bs:readUInt32();            
        resdir.MajorVersion = bs:readUInt16();             
        resdir.MinorVersion = bs:readUInt16();            
        resdir.NumberOfNamedEntries = bs:readUInt16();     
        resdir.NumberOfIdEntries = bs:readUInt16();        

        resdir.Entries = {}


        local cnt = 0;
        while (cnt < resdir.NumberOfNamedEntries + resdir.NumberOfIdEntries) do
            local entry = {
                level = level;
                Name = bs:readUInt32();
                OffsetToData = bs:readUInt32();
            }
            table.insert(resdir.Entries, entry)
            cnt = cnt + 1;
        end
        ...
        for i, entry in ipairs(resdir.Entries) do
            --print(tab, "ENTRY")
...
--]]
    ...
    readResourceDirectory(bs, res, 1, "");
